            ...
            <table className="w-full">
              <thead style={{ backgroundColor: '#f0fdfa' }}>
                <tr>
                  ...
                  <th className="px-4 py-3 text-center font-semibold" style={{ color: '#115e59', fontSize: '14px' }}>계약서</th>
              ...
              <tbody>
                {filteredTemplates.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-4 py-8 text-center text-gray-500" style={{ fontSize: '15px' }}>
                      등록된 템플릿이 없습니다.
                    </td>
                  </tr>
                ) : (
                  filteredTemplates.map((template) => (
                    <tr key={template.id} className="border-t hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() => template.pdf_file_path && window.open(template.pdf_file_path, '_blank')}
                          disabled={!template.pdf_file_path}
                          className={`p-1 rounded transition-colors ${template.pdf_file_path
                              ? 'hover:bg-gray-100 text-[#249689]'
                              : 'text-gray-300 cursor-not-allowed'
                            }`}
                          title={template.pdf_file_path ? "계약서 보기" : "계약서 없음"}
                        >
                          <FileText size={20} />
                        </button>
                      </td>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="border-2 border-dashed rounded-lg p-4" style={{ borderColor: '#e5e7eb' }}>
                  ...
                  />
                  {uploadedPdfFile && (
                    <div className="flex items-center justify-between mt-2 p-2 bg-green-50 rounded border border-green-100">
                      <p className="text-sm text-green-700 truncate">
                        ✓ 새 파일: {uploadedPdfFile.name}
                      </p>
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <div className="flex items-center justify-between mt-2 p-2 bg-gray-50 rounded border border-gray-200">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            if (confirm('기존 계약서 파일을 삭제하시겠습니까? (저장 시 반영됩니다)')) {
                              setFormData({ ...formData, pdf_file_path: null });
                            }
                          }}
                          className="text-red-500 hover:text-red-700 text-sm px-2 flex items-center gap-1"
                        >
                          <Trash2 size={14} /> 삭제
                        </button>
                      </div>
