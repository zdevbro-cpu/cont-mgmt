...
export default function ContractListPage() {
  ...
  const loadContracts = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Î™©Î°ù Ï°∞Ìöå ?§Ìå®');
      }
      ...
      alert('Í≥ÑÏïΩ??Î™©Î°ù??Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§.');
      setContracts([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (contract, event) => {
    event.stopPropagation(); // ???¥Î¶≠ ?¥Î≤§??Î∞©Ï?
    
    if (!confirm(`"${contract.contractor_name}" Í≥ÑÏïΩ????†ú?òÏãúÍ≤†Ïäµ?àÍπå?`)) {
      return;
    }
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="font-bold mb-2" style={{ color: '#000000', fontSize: '18px' }}>
                {filters.search || filters.startDate || filters.minAmount ? 'Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§' : '?±Î°ù??Í≥ÑÏïΩ?úÍ? ?ÜÏäµ?àÎã§'}
              </h3>
              <p className="mb-6" style={{ color: '#6b7280', fontSize: '15px' }}>
                {filters.search || filters.startDate || filters.minAmount ? '?§Î•∏ Ï°∞Í±¥?ºÎ°ú Í≤Ä?âÌï¥Î≥¥ÏÑ∏?? : '??Í≥ÑÏïΩ?úÎ? ?±Î°ù?òÏó¨ Í¥ÄÎ¶¨Î? ?úÏûë?òÏÑ∏??}
              </p>
