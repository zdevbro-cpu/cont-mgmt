...
export default function AdminUsersPage() {
  ...
  const handleApprove = async (user) => {
    if (!confirm(`"${user.full_name}" ?¨Ïö©?êÎ? ?πÏù∏?òÏãúÍ≤†Ïäµ?àÍπå?`)) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || '?πÏù∏ ?§Ìå®');
      }

      alert('?¨Ïö©?êÍ? ?πÏù∏?òÏóà?µÎãà??');
      loadUsers();

    } catch (error) {
      console.error('?πÏù∏ ?§Î•ò:', error);
      alert(error.message || '?πÏù∏?òÎäî???§Ìå®?àÏäµ?àÎã§.');
    }
  };
                    ...
                    <tr key={user.id} 
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${
                          ...
                          {user.role === 'admin' ? 'Í¥ÄÎ¶¨Ïûê' : user.role === 'pending' ? '?πÏù∏?ÄÍ∏? : '?¨Ïö©??}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span style={{ color: '#6b7280', fontSize: '15px' }}>
